...
class ProductivityTests(APITestCase):
    def setUp(self):
        ...
        self.user_a = User.objects.create_user(
            username="user_a", password="password123", email="a@example.com", daily_hour_limit=6.0
        )
        # Crear Usuario B
        ...
        response = self.client.post(
            reverse("login"), {"username": "user_a", "password": "password123"}
        )
        self.token_a = response.data["tokens"]["access"]

        response = self.client.post(
            reverse("login"), {"username": "user_b", "password": "password123"}
        )
    ...
    def test_today_grouping_and_ordering(self):
        ...
        # 3. Hoy (3h)
        Subtask.objects.create(
            activity=self.act_a, name="Hoy 1", target_date=self.today, estimated_hours=3
        )
        # 4. Hoy (1h) - Debería ir primero en su grupo por menor esfuerzo
        Subtask.objects.create(
            activity=self.act_a, name="Hoy 2", target_date=self.today, estimated_hours=1
        )
        # 5. Próxima (mañana, 4h)
        Subtask.objects.create(
            activity=self.act_a,
    ...
    def test_data_isolation_idor(self):
        ...
        Subtask.objects.create(
            activity=self.act_a, name="Privada A", target_date=self.today, estimated_hours=1
        )
    ...
    def test_today_filters(self):
        ...
        Subtask.objects.create(
            activity=self.act_a, name="Math Task", target_date=self.today, estimated_hours=1
        )
        Subtask.objects.create(
            activity=act_fisica, name="Fisica Task", target_date=self.today, estimated_hours=1
        )
    ...
    def test_subtask_create_with_status_and_note(self):
        """Subtask API acepta status/note y sincroniza completed."""
        self.set_auth_a()
        url = "/api/subtasks/"
        payload = {
            "activity": self.act_a.id,
            "name": "Con estado",
            "target_date": str(self.today + timedelta(days=1)),
            "estimated_hours": 2,
            "status": "postponed",
            "note": "Mover por cruce de horarios",
        }

        response = self.client.post(url, payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["data"]["status"], "postponed")
        self.assertEqual(response.data["data"]["note"], "Mover por cruce de horarios")
        self.assertEqual(response.data["data"]["completed"], False)

    def test_subtask_update_status_completed_sets_completed_flag(self):
        """Al actualizar status=completed, completed tambien queda en true."""
        self.set_auth_a()
