...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "days_ahead",
            OpenApiTypes.INT,
            description="Number of days to consider as 'Coming Up'",
            default=7,
        ),
        OpenApiParameter("course", OpenApiTypes.STR, description="Filter by activity course"),
        OpenApiParameter(
            "activity_status", OpenApiTypes.STR, description="Filter by activity status"
        ),
        OpenApiParameter(
            "subtask_status",
            OpenApiTypes.STR,
            description="Filter by subtask status (pending/completed/postponed)",
        ),
        OpenApiParameter(
            "completed", OpenApiTypes.BOOL, description="Filter by subtask completion status"
        ),
    ],
...
def today_view(request):
    ...
    subtask_status_filter = request.query_params.get("subtask_status")
    ...
    if subtask_status_filter:
        subtasks_qs = subtasks_qs.filter(status__iexact=subtask_status_filter)
    ...
    if completed_filter is not None:
        ...
        subtasks_qs = subtasks_qs.filter(completed=False).exclude(status=Subtask.STATUS_COMPLETED)
    ...
    coming_up = subtasks_qs.filter(target_date__gt=today, target_date__lte=limit_date).order_by(
    ...
    return Response(
        {
            ...
            "groups": {
                "overdue": {"title": "Vencidas", "count": overdue.count(), "tasks": overdue_data},
                "today": {"title": "Para hoy", "count": for_today.count(), "tasks": for_today_data},
                "coming_up": {
...
@extend_schema(summary="Get unique courses for the current user", responses={200: OpenApiTypes.STR})
@api_view(["GET"])
@permission_classes([IsAuthenticated])
